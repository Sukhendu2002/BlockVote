...
import {
  setAllRegisteredCanidates,
  setAllRegisteredVoters,
  setDoners,
} from "../../reducer/electionSlice";
  ...
  const loadWeb3 = async () => {
    if (window.ethereum) {
      try {
        ...
        const Allcandidates = await electionInstance.methods
          .getAllCandidates()
          .call();
        // dispatch(setAllRegisteredCanidates(Allcandidates));
        setAllCurrentCandidates(Allcandidates);

        const AllDonations = await electionInstance.methods
          .getDonationDetails()
          .call();
        ...
        <>
          {currentVoter.isRegistered ? (
            ...
            <div className="container-item attention">
              {!isAdmin ? (
                <center>
                  <span className="p-2 rounded-lg bg-[#f2c8c8] block text-[#424242] w-full">
                ...
                <div className="bg-[#6bf5c0] rounded p-2">
                  <h1 className="text-center text-5xl text-[#4e4e4e]">
                    Welcome Admin
                  </h1>
                </div>
              )}
          ...
          <div className="voting-can mt-8">
            {candidates.length < 1 ? (
              ...
              <FundGraph
                allCurrentCandidates={allCurrentCandidates}
                allVoters={allCurrentVoters}
                allCurrentdoners={allCurrentdoners}
              />
            ) : (
              <FundDonate
                allCurrentCandidates={allCurrentCandidates}
