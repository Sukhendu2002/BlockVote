...
const DonationHistory = ({
  ...
  const { doners, allRegisteredCanidates } = useSelector(
  ...
  useEffect(() => {
    if (allCurrentdoners.length > 0) {
      const temp = allCurrentdoners.filter(
        (doner) => doner.donorAddress === currentVoter
      );
  ...
  return (
    <div className="mt-8 w-full">
      ...
      <table className="border-[1px] w-1/2 text-center mx-1o mt-10 bg-[#b6b6b6]">
          ...
          donationHistory.map((donation, index) => (
            <tr className="border-[1px]" key={index}>
              <td className="border-[1px]">
                {allCurrentCandidates[donation.donetedTo].header}
              </td>
              <td className="border-[1px]">
                {Web3.utils.fromWei(donation.amount, "ether")}
              </td>
            </tr>
          ))}
      </table>
