...
const FundGraph = ({ allCurrentCandidates, allVoters, allCurrentdoners }) => {
  const { doners, allRegisteredCanidates, allRegisteredVoters } = useSelector(
    (state) => state.electionSlice
  );
  ...
  useEffect(() => {
    const amounts = allCurrentCandidates.map((candidate, index) => {
      const candidateDoners = allCurrentdoners.filter(
        (doner) => parseInt(doner.donetedTo) === index
      );
      let amount = 0;
    ...
    setAmountPerCandidate(
      amounts.map((item) => Web3.utils.fromWei(item.toString(), "ether"))
    );
    console.log(allCurrentdoners);
  ...
  return (
    <div className="w-[700px] h-[800px]">
      <h3 className="mx-auto text-center">
        {" "}
        visit{" "}
        <Link to="/dashboard" className="underline hover:text-[#4da5b9]">
          Dashboard.
        </Link>
      </h3>
      <Bar
