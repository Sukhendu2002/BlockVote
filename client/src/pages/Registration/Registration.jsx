...
import { setIsRegistredVoter } from "../../reducer/electionSlice";
...
const Registration = () => {
  ...
  const loadWeb3 = async () => {
    if (window.ethereum) {
      try {
        ...
        dispatch(setIsRegistredVoter(voter));
        // console.log(voter);
  ...
  const registerAsVoter = async (event) => {
    ...
    await axios
      .post("https://128.199.16.111/post-face", formData, {
        headers: {
          "Content-Type": "multipart/form-data",
        },
      })
    ...
    await axios
      .post("https://128.199.16.111/check-face", formData, {
        headers: {
          "Content-Type": "multipart/form-data",
        },
      })
      .then(async (res) => {
        if (res.data.result[0]._label === voterData?.email) {
          verified = true;
          await axios
            .post("https://128.199.16.111/send", {
    ...
    <Container>
      ...
      {!isElStarted && !isElEnded ? (
        ...
        <>
          ...
          >
            <p className="text-[#0168CF] text-6xl mb-4">
              Total Registered Voters:{" "}
              <strong className="text-[#171717]">{voterCount}</strong>
            </p>
            <span>
              see all donated funds.{" "}
              <Link to="/dashboard" className="underline hover:text-[#1d5483]">
                here
              </Link>
            </span>
          </div>
...
export function loadCurrentVoter(voter, isRegistered) {
  return (
    <>
      ...
      >
        <center className="text-5xl my-4 font-bold text-[#0168CF]">
